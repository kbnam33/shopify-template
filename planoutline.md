I've read all three files in full. Here is the complete technical plan for OpenCode to execute.

---

# NEERA THEME — TECHNICAL PLAN
...
**What this plan does:**
1. Adds a `hero_breathing_animation` setting to `settings_schema.json`
2. Modifies `snippets/theme-styles-variables.liquid` to output a `@keyframes` breathing animation CSS variable
3. Modifies `sections/hero.liquid` to support the breathing animation and right-aligned editorial text layout
4. Modifies `sections/header-announcements.liquid` for a clean single-line announcement bar
5. Modifies `sections/marquee.liquid` for the promo/scrolling strip
6. Modifies `sections/collection-links.liquid` for the featured collections grid
7. Creates `sections/email-signup-custom.liquid` as a new standalone email signup section
8. Modifies `templates/index.json` to set the correct section order

**Zero hardcoding rule:** Every color, font, text, and spacing value pulls from `settings.*` or CSS variables already defined in `theme-styles-variables.liquid`.
...
## AFTER EXECUTION — VERIFICATION CHECKLIST

Once OpenCode runs all tasks, verify in the live preview (`http://127.0.0.1:9292`):

- [ ] Announcement bar shows at the top with uppercase tracking
- [ ] Hero image slowly breathes (scale pulse)
- [ ] Hero text sits bottom-right with overlay visible
- [ ] Marquee strip scrolls smoothly below hero
- [ ] Collection grid shows correct columns on desktop and 2 on mobile
- [ ] Email signup renders with form, submits correctly
- [ ] All sections appear in Shopify Theme Editor under "Customize" with their settings visible
- [ ] No hardcoded colors anywhere — all pulling from color scheme

---

Paste this plan into OpenCode and let it execute. Once done, push to Shopify with:

```powershell
shopify theme push --store x9iqze-nb.myshopify.com
```

Then share what you see and we'll move to Phase 2.